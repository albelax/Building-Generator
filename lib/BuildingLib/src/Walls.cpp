...
{
	/// from the rule it generates the matrices necessary
	/// to make the shape of the building
	...
	{
		/// checks the current letter of the rule with the previous one
		/// to determine the rotation and translation of the matrix
		switch(m_rule[i])
		{
		...
		if ( x != 0 ) { correction_x = x > 0 ? -.5f : 0.5f; }
